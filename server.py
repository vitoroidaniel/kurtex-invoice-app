...
@admin_required
def api_delete_all_entries():
    """Wipe ALL oil change records (admin roles only)."""
    save_entries([])
    return jsonify({"ok": True})
...
# ── Serve static files ────────────────────────────────────────────────────
# Layout: static/pages/*.html, static/css/*.css, static/js/*.js,
#         static/icons/*.png, static/manifest.json, static/service-worker.js
...
def serve_admin_login():
    return send_from_directory(str(PAGES_DIR), "admin-login.html")
...
@app.route("/web")
def serve_web():
    # /web is retired — the mobile app at / is the single app for mobile + PC
